...
use crate::{error::ContractError, structs::LotteryState};
use cosmwasm_std::{Addr, DepsMut, Storage, Coin};
...
use super::state_entries::{ID_CURRENT_LOTTERY, ENTRY_PRICE, PRIZE_POOL_SIZES, PRIZES};
...
pub fn update_lottery_state(deps: DepsMut, new_state: LotteryState) -> Result<(), ContractError> {
...
pub fn register(deps: DepsMut, caller: Addr) -> Result<(), ContractError> {
    ...
    USER_STATE.save(deps.storage, (&id_current_lottery.to_string(), &caller.to_string(), "registration"), &true)?;
...
pub fn start_new_lottery(deps: DepsMut, new_id: u32) -> Result<(), ContractError> {
    ...
    LOTTERY_STATE.save(deps.storage, &LotteryState::Registration)?;
...
pub fn set_prize_pool(deps: DepsMut, target_id: u32, prizes: Vec<PrizeRegistered>) -> Result<(), ContractError> {
    ...
    for id_prize in 0..nb_prizes as usize {
        PRIZES.save(deps.storage, (lottery_id, &id_prize.to_string()), &prizes[id_prize])?;
    }
